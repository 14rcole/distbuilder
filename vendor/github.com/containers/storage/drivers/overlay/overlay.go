...
func Init(home string, options []string, uidMaps, gidMaps []idtools.IDMap) (graphdriver.Driver, error) {
	...
	logrus.Debug("getting root UID and GID...")
	rootUID, rootGID, err := idtools.GetRootUIDGID(uidMaps, gidMaps)
	if err != nil {
		return nil, err
	}

	logrus.Debug("Creating driver home dir...")
	// Create the driver home dir
	if err := idtools.MkdirAllAs(path.Join(home, linkDir), 0700, rootUID, rootGID); err != nil && !os.IsExist(err) {
		return nil, err
	}

	logrus.Debug("checking that home dir supports overlay")
	...
	logrus.Debug("making home dir private")
	...
	logrus.Debug("Creating new diff driver")
...
func supportsOverlay(home string, homeMagic graphdriver.FsMagic, rootUID, rootGID int) (supportsDType bool, err error) {
	...
	if err == nil {
		...
		logrus.Debugf("flags: %q", flags)
		if len(flags) < unix.Getpagesize() {
			logrus.Debugf("mount from...")
			err := mountFrom(filepath.Dir(home), "overlay", mergedDir, "overlay", 0, flags)
			logrus.Debugf("mounted")
			if err == nil {
