...
func (b *Builder) pullImage(image string) (types.ImageReference, error) {
	spec := image
	sc := getSystemContext(&types.SystemContext{}, "")
	srcRef, err := alltransports.ParseImageName(spec)
	if err != nil {
		transport := DefaultTransport
		spec = transport + image
	...
	destName, err := localImageNameForReference(b.store, srcRef, spec)
	if err != nil {
	...
	destRef, err := is.Transport.ParseStoreReference(b.store, destName)
	if err != nil {
		return nil, errors.Wrapf(err, "error parsing image name %q", destName)
	}

	img, err := srcRef.NewImageSource(nil, sc)
	if err != nil {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "error creating a new signature policy context")
	}
	...
	err = cp.Image(nil, policyContext, destRef, srcRef, getCopyOptions(os.Stderr, sc, nil, ""))
	if err == nil {
		return destRef, nil
	}
	return nil, err
...
func getImageDigest(src types.ImageReference, ctx *types.SystemContext) (string, error) {
	newImg, err := src.NewImage(nil, ctx)
	if err != nil {
		return "", err
	}
	defer newImg.Close()
