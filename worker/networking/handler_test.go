...
func TestHealth(t *testing.T) {
	...
	if rr.Body.String() != expected {
		t.Errorf("handler returned unexpected body: got %q wanted %q", rr.Body.String(), expected)
	}
}
...
func TestBuild(t *testing.T) {
	...
	if !results.Success {
		t.Errorf("Could not build container: %q", results.Error)
	}
}
